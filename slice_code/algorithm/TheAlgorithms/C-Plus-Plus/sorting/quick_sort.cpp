 ...
 * in different ways.
 *
 *      1. Always pick the first element as pivot // @slice: 选择首元素为支点
 *      2. Always pick the last element as pivot (implemented below) // @slice: 选择尾元素为支点
 *      3. Pick a random element as pivot // @slice: 随机选择支点
 *      4. Pick median as pivot // @slice: 选择中点作为支点
 *
 ...
 * @param low first point of the array (starting index) // @slice: 左坐标
 * @param high last point of the array (ending index) // @slice: 右坐标
 * @returns index of the smaller element
 */
template <typename T>
int partition(std::vector<T> *arr, const int &low, const int &high) {
    T pivot = (*arr)[high];  // taking the last element as pivot // @slice: 尾元素为支点
