 ...
 * @dst: The folio to migrate the data to.      // @slice: 源folio
 * @src: The folio containing the current data. // @slice: 目的folio
...
#ifdef CONFIG_BLOCK // @slice: Block Buffer宏
		...
		struct folio *dst, struct folio *src, enum migrate_mode mode)
{
	...
	if (mode != MIGRATE_SYNC_NO_COPY)
		...
		folio_migrate_flags(dst, src); // @slice: Sync no copy模式: 只合并flag
	return MIGRATEPAGE_SUCCESS;
}
				...
				enum migrate_mode mode)
{
	...
	if (likely(is_lru)) {
		...
		if (!mapping)
			 ...
			 * provide a migrate_folio callback. Anonymous folios // @slice: 匿名folio属于swap空间
			       ...
			       enum migrate_reason reason, struct list_head *ret)
{
	...
	*dstp = dst;
	...
	if (!folio_trylock(src)) {
		if (mode == MIGRATE_ASYNC) // @slice: 异步合并直接out
			goto out;
...
#ifdef CONFIG_NUMA_BALANCING // @slice: NUMA平衡相关函数
/*
