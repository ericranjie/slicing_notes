...
bool veri(string a, string b) { // Verify: same-size; POE:
    if (a == b) { // Guard: Brother word must not same; POE:
        return false;
    }
...
int main() {
    ...
    vector<string> r; // Raw dictionary; // POE:
    for (int i = 0; i < n; i++) {
        cin >> t;
        r.push_back(t);
    }
    ...
    vector<string> dst; // Dictionary set
    for (auto it = r.begin(); it != r.end(); it++) {
        string tmp = *it;
        if (tmp.size() != t.size()) // Guard: Character number not same;
            continue;
         else  // Main: Character number same;
            if (!veri(t, tmp))  // POE:
                continue;
